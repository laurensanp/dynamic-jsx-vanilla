...
export function updateStats(filteredLogs, errorCount, warnCount, infoCount, debugCount) {
...
export function filterLogs(logs, logOutput, levelFilter, moduleFilter, searchInput, errorCount, warnCount, infoCount, debugCount, renderLogs) {
...
export function downloadLogs(logs) {
  const logContent = logs.map(log => 
    `[${log.timestampText}] [${log.level.toUpperCase()}]${log.module ? ` [${log.module.toUpperCase()}]` : ''} ${log.message}`
  ).join('\n');
