...
export const addOutput = (testOutput, message, type = "info") => {
  if (
    type === "error" &&
    message.includes(
      "Anfragen führten zu erwartetem 401 Unauthorized/Ratelimit"
    )
  ) {
    return;
  }
...
export function updateTestItemUI(
  testSuites,
  liveTestResults,
  testName,
  statusClass,
  ...
  items.forEach((item) => {
    ...
    const suite = testSuites.find((s) =>
      s.tests.some((t) => t.name.trim() === testName.trim())
    );
    const test =
      suite && suite.tests.find((t) => t.name.trim() === testName.trim());
...
export const resetTestResultsUI = (
  testOutput,
  liveTestResults,
  testSuites,
  updateStatsDisplay,
  ...
  liveTestResults.pending = testSuites.reduce(
  ...
  addOutput(
    testOutput,
    "Test-Suite bereit. Klicken Sie auf „Alle Tests starten“, um zu beginnen."
  );
};
