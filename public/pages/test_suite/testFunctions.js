...
export const testFunctions = {
  ...
  "POST /api/v1/auth": async () => {
    await fetch("/api/v1/meta/reset-login-attempts", {
      method: "POST",
    ...
    return executeFetchTest(
      "/api/v1/auth",
      {
        ...
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          username: TestSettings.TEST_USERNAME,
  ...
  "Cache Layer": async () => {
    ...
    const result = await executeFetchTest(
      "/api/v1/hello",
      { method: "GET" },
      200,
      (res, data) => res.ok
  ...
  //   return {
  //     success:
  //       successCount === TestSettings.CONCURRENT_USERS_COUNT && duration < 2000,
  //     status: 200,
  //     message: `${successCount}/${
  //       TestSettings.CONCURRENT_USERS_COUNT
  //     } Anfragen erfolgreich in ${Math.round(duration)}ms`,
  //     expectedStatus: 200,
  //   };
  // },
    ...
    for (let i = 0; i < NUM_REQUESTS; i++) {
      promises.push(
        executeFetchTest(
          ...
          { method: "GET" },
          200,
          (res, data) => res.ok && res.status === 200
        )
      );
    }
    const results = await Promise.all(promises);
  ...
  "Full User CRUD Test": async () => {
    ...
    try {
      ...
      if (!readResult.success)
        throw new Error(
          readResult.message || "Failed to read user or data mismatch"
        );
      ...
      if (!checkUpdateResult.success)
        throw new Error(
          checkUpdateResult.message || "Update verification failed"
        );
      ...
      if (userId) {
        try {
          await fetch(`/api/v1/meta/cleanup-user/${userId}`, {
            method: "POST",
          });
        } catch (e) {}
      }
  ...
  "Data Integrity Test": async () => {
    ...
    try {
      ...
      if (!createResult.success)
        throw new Error(createResult.message || "Failed to create data");
      ...
      if (!readResult.success)
        throw new Error(
          readResult.message || "Failed to read data or data mismatch"
        );
      ...
      if (!updateResult.success)
        throw new Error(updateResult.message || "Failed to update data");
      ...
      if (!deleteResult.success)
        throw new Error(deleteResult.message || "Failed to delete data");
      ...
      if (dataId) {
        try {
          await fetch(`/api/v1/meta/cleanup-data/${dataId}`, {
            method: "POST",
          });
        } catch (e) {}
      }
  ...
  "Large Data Payload Test": async () => {
    const largeData = {
      content: "a".repeat(TestSettings.LARGE_DATA_PAYLOAD_SIZE),
    };
    ...
    try {
      ...
      const createResult = await executeFetchTest(
        ...
        (res, data) =>
          res.ok &&
          data.data &&
          data.data.content.length === largeData.content.length
      );

      if (!createResult.success)
        throw new Error(
          createResult.message ||
            "Failed to send large data or data not processed correctly"
        );
      ...
      if (
        ...
        await fetch(`/api/v1/data/${createResult.data.data.id}`, {
          method: "DELETE",
        });
      }
  ...
  "Resource Exhaustion Test": async () => {
    ...
    try {
      ...
      for (let i = 0; i < NUM_OPERATIONS; i++) {
        const createResult = await executeFetchTest(
          "/api/v1/users",
          {
            ...
            body: JSON.stringify({
              ...
              email: `exhaustion_user_${i}@example.com`,
            }),
          },
          200,
          (res) => res.ok
        ...
        if (!createResult.success)
          throw new Error(createResult.message || `Failed to create user ${i}`);
        ...
        if (!deleteResult.success)
          throw new Error(deleteResult.message || `Failed to delete user ${i}`);
      }
...
export async function executeFetchTest(
  url,
  options = {},
  expectedStatus = 200,
  successCondition = (res, data) => res.ok
) {
...
export async function loginTestUser() {
  ...
  if (!response.ok) {
    return {
      success: false,
      message: `Failed to login test user: ${response.statusText}`,
    };
  }
