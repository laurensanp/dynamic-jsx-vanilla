...
describe("Integrationstests", () => {
  ...
  test("Externe API", async () => {
    const res = await auth(
      request(app).get("/api/v1/shut").set("x-test-mode", "true")
    );
    expect(res.status).toBe(200);
  ...
  test("Concurrent User Creation (integration)", async () => {
    ...
    for (let i = 0; i < NUM_USERS; i++) {
      ...
      createUsersPromises.push(
        auth(request(app).post("/api/v1/users").send(testUser))
      );
    }
    const createResponses = await Promise.all(createUsersPromises);
    expect(createResponses.filter((res) => res.status === 201).length).toBe(
      NUM_USERS
    );
  });
});
