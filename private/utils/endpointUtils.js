...
module.exports = {
  ...
  getHealthChecks: async () => {
    const api = {
      ok: true,
      message: ServerEndpointSettings.HEALTH_API_RESPONDING,
    };

    let database = {
      ok: false,
      message: ServerEndpointSettings.HEALTH_DATABASE_UNAVAILABLE,
    };
    try {
      ...
      database = hasUsers
        ? {
            ok: true,
            message: ServerEndpointSettings.HEALTH_DATABASE_AVAILABLE(
              testUsers.length
            ),
          }
        : {
            ok: false,
            message: ServerEndpointSettings.HEALTH_DATABASE_MISSING,
          };
    } catch (e) {
    ...
    let cacheCheck = {
      ok: false,
      message: ServerEndpointSettings.HEALTH_CACHE_PING_FAILED,
    };
    try {
      const ok = await cache.ping();
      cacheCheck = ok
        ? { ok: true, message: ServerEndpointSettings.HEALTH_CACHE_OK }
        : {
            ok: false,
            message: ServerEndpointSettings.HEALTH_CACHE_PING_FAILED,
          };
    ...
    let monitoring = {
      ok: false,
      message: ServerEndpointSettings.HEALTH_MONITORING_NOT_ACCESSIBLE,
    };
    try {
      ...
      if (exists) {
        ...
        monitoring = {
          ok: true,
          message: ServerEndpointSettings.HEALTH_MONITORING_AVAILABLE(
            stat.size
          ),
        };
      } else {
        monitoring = {
          ok: false,
          message: ServerEndpointSettings.HEALTH_MONITORING_MISSING,
        };
      }
    } catch (e) {
    ...
    const overall =
      okCount === ServerEndpointSettings.HEALTH_OK_THRESHOLD
        ? "ok"
        : okCount >= ServerEndpointSettings.HEALTH_DEGRADED_THRESHOLD
        ? "degraded"
        : "down";
