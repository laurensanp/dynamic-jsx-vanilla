...
router.get(ServerAuthSettings.LOGIN_ENDPOINT, (req, res) => {
  if (console.debug) console.debug(`[LOGIN] ${ServerAuthSettings.LOGIN_ENDPOINT} GET called by ${req.userIp || "unknown IP"}`);
...
router.get(ServerAuthSettings.LOGOUT_ENDPOINT, isAuthenticated, (req, res) => {
  if (console.debug) console.debug(`[LOGOUT] ${ServerAuthSettings.LOGOUT_ENDPOINT} GET called by ${req.userIp || "unknown IP"}`);
...
router.post(ServerAuthSettings.AUTH_POST_ENDPOINT, checkRateLimit, (req, res) => {
  if (console.debug) console.debug(`[AUTH] ${ServerAuthSettings.AUTH_POST_ENDPOINT} POST called by ${req.userIp || "unknown IP"} with body: ${JSON.stringify(req.body)}`);
  const { username, password } = req.body;

  if (!username || !password) {
    if (console.warn && console.warn.api) console.warn.api(`[WARN] ${ServerAuthSettings.AUTH_POST_ENDPOINT} POST missing username or password: username=${username}, password=${password}`);
    return res.status(400).json({ success: false, message: ServerAuthSettings.USERNAME_PASSWORD_REQUIRED_MESSAGE });
  }
  ...
  if (username === validUsername && password === validPassword) {
    resetLoginAttempts(req.userIp);
    res.json({ success: true, message: ServerAuthSettings.AUTHENTICATION_SUCCESS_MESSAGE });
  } else {
    loginAttempts[req.userIp].count++;
    res.status(401).json({ success: false, message: ServerAuthSettings.INVALID_CREDENTIALS_MESSAGE });
  }
});
